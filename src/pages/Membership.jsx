  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Join IAPEN India. Discover membership benefits, fees, and apply online to become a part of India's leading clinical nutrition community." />
      </Helmet>
        ...
        <div className="container">
          <div className="grid-2 align-start">
            <div>
              ...
              <div className="benefits-columns mt-6" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
                <div className="glass-panel" style={{ padding: '24px', borderRadius: 'var(--radius-lg)', background: 'linear-gradient(145deg, rgba(255,255,255,0.9), rgba(240,245,250,0.5))', border: '1px solid rgba(11,60,93,0.1)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ padding: '10px', background: 'var(--primary-light)', borderRadius: '12px', color: 'var(--primary)' }}><Briefcase size={24} /></div>
                    <h3 style={{ fontSize: '18px', color: 'var(--primary-navy)', margin: 0 }}>Life Membership</h3>
                  </div>
                  <ul className="benefits-checklist" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {lifeBenefits.map((benefit, idx) => (
                      <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', fontSize: '14px', color: 'var(--text-muted)' }}>
                ...
                <div className="glass-panel" style={{ padding: '24px', borderRadius: 'var(--radius-lg)', background: 'linear-gradient(145deg, rgba(255,255,255,0.9), rgba(242,250,254,0.5))', border: '1px solid rgba(32,128,168,0.1)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                    <div style={{ padding: '10px', background: 'var(--teal-light)', borderRadius: '12px', color: 'var(--teal)' }}><User size={24} /></div>
                    <h3 style={{ fontSize: '18px', color: 'var(--teal)', margin: 0 }}>Student Membership</h3>
                  </div>
                  <ul className="benefits-checklist" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {studentBenefits.map((benefit, idx) => (
                      <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', fontSize: '14px', color: 'var(--text-muted)' }}>
                        <Check size={16} style={{ color: 'var(--teal)', flexShrink: 0, marginTop: '2px' }} />
                        <span>{benefit}</span>
                      </li>
                    ))}
                  </ul>
