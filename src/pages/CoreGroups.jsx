...
import { Search, Info, Activity, Filter, ChevronDown, CheckCircle, X } from 'lucide-react';
...
const CoreGroups = () => {
  ...
  const categories = ['All', 'Clinical Specialities', 'Geriatric & Pediatrics', 'Surgical & Critical Care', 'Public & Integrative Health'];
  ...
  const coreGroupsData = [
    {
      ...
      focus: ['MNT for Type 1 & 2 Diabetes', 'Gestational Diabetes Guidelines', 'Public awareness campaigns'],
    ...
    },
    {
      ...
      focus: ['Immunosuppressant dietary adjustments', 'Pre-transplant optimization', 'Post-operative recovery diets'],
    ...
    },
    {
      ...
      focus: ['Texture-modified food standards', 'Dysphagia screening tools', 'Multidisciplinary feeding clinics'],
    ...
    },
    {
      ...
      focus: ['Pre-operative fasting reduction', 'Early post-op oral intake', 'Perioperative fluid management'],
    ...
    },
    {
      ...
      focus: ['DASH diet localization', 'Sodium reduction campaigns', 'Cardiac rehabilitation protocols'],
    ...
    },
    {
      ...
      focus: ['Cancer cachexia prevention', 'Chemo-induced nausea management', 'Enteral nutrition in head/neck cancers'],
    ...
    },
    {
      ...
      focus: ['Neonatal TPN guidelines', 'Pediatric ICU feeding protocols', 'Malnutrition management standards'],
    ...
    },
    {
      ...
      focus: ['Undernutrition rehabilitation', 'Anemia eradication projects', 'School feeding program standards'],
    ...
    },
    {
      ...
      focus: ['Parenteral nutrition monitoring', 'Early enteral feeding in sepsis', 'ICU calorimetry standards'],
    ...
    },
    {
      ...
      focus: ['CKD stage-specific menus', 'Dialysis nutrition guidelines', 'Renal enteral formula criteria'],
    ...
    },
    {
      ...
      focus: ['Tele-nutrition protocols', 'Diet software guidelines', 'Mobile health app validation'],
    ...
    },
    {
      ...
      focus: ['Pre-conception dietary support', 'Lactation nutritional guidelines', 'Micro-nutrient supplementation'],
    ...
    },
    {
      ...
      focus: ['Lifestyle modification programs', 'Anti-inflammatory diets', 'Fatty liver nutritional care'],
    ...
    },
    {
      ...
      focus: ['Sarcopenia prevention diets', 'Cognitive nutrition standards', 'Geriatric screening tools'],
    ...
    },
    {
      ...
      focus: ['Low-FODMAP diets', 'Celiac-safe nutrition standards', 'Short bowel syndrome management'],
    ...
    },
    {
      ...
      focus: ['Ketogenic diet for epilepsy', 'Post-stroke enteral feeding', 'Cognitive support nutrition'],
      ...
      <section className="section core-groups-section">
        <div className="container">
          <div className="section-title-wrapper">
            ...
            <p className="section-desc">IAPEN India hosts 17 voluntary core groups focused on researching, reviewing, and publishing guidelines for disease-specific clinical nutrition.</p>
          </div>
              ...
              return (
                ...
                >
                  <div className="group-card-header">
                    ...
                    {!hasDetails && <ChevronDown size={18} className={`expand-chevron ${isExpanded ? 'rotate' : ''}`} />}
                  </div>
          ...
          {filteredGroups.length === 0 && (
            <div className="no-results-box text-center glass-panel">
              ...
              <p className="text-muted">Try searching with a different keyword or selecting a different category.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {selectedGroup && coreGroupsCommittees[selectedGroup.name] && (
        <div className="modal-backdrop" onClick={() => setSelectedGroup(null)}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '850px' }}>
            <button className="modal-close" onClick={() => setSelectedGroup(null)} aria-label="Close modal">
              <X size={24} />
            </button>
            <div className="modal-body-content">
              <div className="modal-member-header" style={{ marginBottom: '16px', paddingBottom: '16px' }}>
                ...
                <div>
                  ...
                  <span className="custom-badge bg-teal-light" style={{ fontSize: '13px' }}>{selectedGroup.category}</span>
                </div>
              </div>

              <div style={{ maxHeight: '60vh', overflowY: 'auto', paddingRight: '10px', display: 'flex', flexDirection: 'column', gap: '24px' }} className="text-left">
                {/* Description */}
                <div>
                  <p style={{ fontSize: '15px', color: 'var(--text-muted)', lineHeight: '1.6' }}>{selectedGroup.desc}</p>
                </div>

                {/* Aims, Vision, Mission (if present, like in Pediatric Core Group) */}
                {coreGroupsCommittees[selectedGroup.name].aim && (
                  <div style={{ backgroundColor: 'var(--bg-section)', padding: '20px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-ultra-light)' }}>
                    <h4 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--primary-dark)', marginBottom: '10px' }}>Aims, Vision & Mission</h4>
                ...
                {/* Committee Team */}
                <div>
                  <h4 style={{ fontSize: '18px', fontWeight: '700', color: 'var(--primary-dark)', borderBottom: '2px solid var(--teal-light)', paddingBottom: '6px', marginBottom: '16px' }}>
                    Core Committee Members
                  </h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    {coreGroupsCommittees[selectedGroup.name].members.map((member, idx) => (
                      <div key={idx} style={{ borderBottom: idx < coreGroupsCommittees[selectedGroup.name].members.length - 1 ? '1px dashed var(--border-ultra-light)' : 'none', paddingBottom: '16px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '8px' }}>
                          <strong style={{ fontSize: '16px', color: 'var(--primary-dark)' }}>{member.name}</strong>
                          <span className="custom-badge bg-teal-light" style={{ fontSize: '12px' }}>{member.role}</span>
                        </div>
                ...
                {coreGroupsCommittees[selectedGroup.name].launchActivity && (
                  <div style={{ borderTop: '1px solid var(--border-ultra-light)', paddingTop: '16px' }}>
                    <h4 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--primary-dark)', marginBottom: '8px' }}>Scientific Activities & Launch</h4>
                    <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: '1.6' }}>{coreGroupsCommittees[selectedGroup.name].launchActivity}</p>
                  </div>
                )}
              </div>
            </div>
