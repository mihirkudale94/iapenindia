...
const geoUrl =
  'https://raw.githubusercontent.com/gramener/mapviewer/init/test/india-states.geojson';
...
const getMemberIdentity = (title) => {
  ...
  return { role: 'Committee Member', name: title };
};

const chaptersCommittees = Object.fromEntries(
  officialChapters.map((chapter) => [
    chapter.name,
    {
      ...chapter,
      members: chapter.members.map((member) => ({
        ...getMemberIdentity(member.title),
...
const MapChart = ({ chapters, selectedState, onStateClick }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h4
          style={{ margin: 0, fontSize: '13px', color: 'var(--primary-dark)', fontWeight: '700' }}
        >
...
// Kept temporarily for compatibility with archived committee data.
...
const parseDetails = (detailsText) => {
  ...
  const splitRegex =
    /(AFFILIATION|DESIGNATION|EXPERIENCE|WORK EXPERIENCE|ASSOCIATION WITH OTHER[S]? SOCIETIES|ASSOCIATION WITH OTHER[S]? ASSOCIATIONS?|ASSOCIATION WITH OTHER[S]? ORGANIZATIONS?|ASSOCIATION WITH OTHER[S]? SOCIETIES AND ASSOCIATIONS|ACHIEVEMENTS|ACHIEVEMENTS AND ACCOLADES|AWARDS AND CERTIFICATES|AWARDS|AREAS OF SPECIAL INTEREST|AREAS OF INTEREST|FIELD OF INTEREST IN NUTRITION|FIELD OF INTEREST|SPECIAL INTERESTS)\s*[-:]\s*/g;
        ...
        >
          {hasParsedFields ? (
            ...
            <p style={{ margin: 0, color: 'var(--text-muted)' }}>
              {member.details || 'No biography details provided.'}
            </p>
          )}
        </div>
      )}
...
const Chapters = () => {
  ...
  const filteredChapters = chaptersData.filter((chapter) => {
    const officialChapter = chaptersCommittees[chapter.name];
    const matchesSearch =
      chapter.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      chapter.state.toLowerCase().includes(searchTerm.toLowerCase()) ||
      officialChapter?.members.some((member) =>
        member.name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    const matchesRegion = filterRegion === 'All' || chapter.region === filterRegion;
    const matchesState =
      !selectedState || chapter.state.toLowerCase() === selectedState.toLowerCase();
    ...
    <div className="chapters-page animate-slide-up">
        ...
        <div className="container">
          ...
          <div className="search-filter-container glass-panel">
            <div className="search-box-wrapper">
              ...
              <input
                type="text"
                placeholder="Search chapter, state, or member..."
          ...
          <div id="directory" className="grid-3 chapters-grid">
            {filteredChapters.map((chapter, index) => {
              const officialChapter = chaptersCommittees[chapter.name];
              const hasDetails = !!officialChapter;
              return (
                <div
                  key={index}
                ...
                >
                  ...
                  <div className="chapter-body">
                    ...
                    <div className="chapter-detail-item">
                      ...
                      <span>Meet {officialChapter.members.length} official members</span>
                    </div>
                    <div className="chapter-detail-item">
                      <Mail size={16} className="detail-icon" />
                      <span className="chapter-email-link">
                        {officialChapter.email || chapter.email}
                      </span>
                    </div>
                    {hasDetails && (
      ...
      {selectedChapter &&
        createPortal(
          (() => {
            try {
              const cleanedMembers = selectedChapter.members;
              const officeBearers = cleanedMembers.filter(
                (m) =>
                  m.role &&
                ...
                <div className="modal-backdrop" onClick={() => setSelectedChapter(null)}>
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ maxHeight: '75vh', overflowY: 'auto', paddingRight: '5px' }}
                    >
                      <div
                      ...
                      >
                        ...
                        <div>
                          ...
                          >
                            Region: {selectedChapter.region} India &bull; State:{' '}
                            {selectedChapter.state}
                          </p>
                          <Link
                            to={`/chapters/${chapterSlug(selectedChapter.name)}`}
                      ...
                      {officeBearers.length > 0 && (
                        <div
                          className="chapter-committee-section text-left"
                              ...
                              return (
                                ...
                                >
                                  <div
                                    ...
                                    onClick={() =>
                                      setExpandedMember(isExpanded ? null : `office-${idx}`)
                                    }
                                  >
                                    <div>
                                      <strong
                              ...
                              return (
                                ...
                                >
                                  <div
                                    ...
                                    onClick={() =>
                                      setExpandedMember(isExpanded ? null : `exec-${idx}`)
                                    }
                                  >
                                    <div>
                                      <strong
                  ...
                  >
                    ...
                    <div style={{ color: '#b91c1c', textAlign: 'left' }}>
                      ...
                      <p style={{ fontSize: '14px', marginBottom: '15px' }}>
                        An error occurred while loading this chapter's committee details.
                      </p>
                      <pre
                        style={{
